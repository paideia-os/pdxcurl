#!/usr/bin/env bash
# Per-repo build script. Mirrors pdxsock/tools/build.sh and
# mkfs.pdxfs/tools/build.sh (paideia-os issues #1976/#1977 --
# satellite-tool /bin seeding pipeline). Runs paideia-as build over
# every .pdx source, then links this repo's own objects into a flat
# ELF via `ld -T link.ld`.
#
# Resolves paideia-as via (in order):
#   1. $PAIDEIA_AS env var
#   2. paideia-os checkout sibling:
...
#
# Requires paideia-as >= 0.36.0 (v1.1-B: sys_semantic_send emission
# uses movabs u64 schema constant + rip-relative record store, both
# stable in 0.36).
#
# Usage:
...
if [ "$FAIL" -eq 0 ] && [ "${#OWN_OBJECTS[@]}" -gt 0 ]; then
    echo "[link] ld -T link.ld -> $BUILD_DIR/pdxcurl.elf"
    ld -nostdlib --warn-common --fatal-warnings --gc-sections \
        -T link.ld \
        -o "$BUILD_DIR/pdxcurl.elf" \
        "${OWN_OBJECTS[@]}" "${EXTRA_OBJECTS[@]}" "${EXTRA_ARCHIVES[@]}"
    echo "[link] OK -> $BUILD_DIR/pdxcurl.elf"

    objcopy -O binary "$BUILD_DIR/pdxcurl.elf" "$BUILD_DIR/pdxcurl.bin"
    echo "[link] OK -> $BUILD_DIR/pdxcurl.bin"
fi
